...
class ImageData:
    ...
    def resize_image(self, pad: bool = True, mode: str = 'pad'):
        ...
        if mode == 'stretch':
            self.transformed_image = cv2.resize(self.transformed_image,
                                                dsize=(self.const_width,
                                                       self.const_height))
        else:
            ...
            self.transformed_image = cv2.resize(self.transformed_image, (0, 0),
            ...
            if pad:
                ...
                if w < cw:
                    self.transformed_image = cv2.copyMakeBorder(
                        self.transformed_image, 0, 0, 0, cw - w,
                        cv2.BORDER_CONSTANT)
                    self.left_border = cw - w
                if h < ch:
                    self.transformed_image = cv2.copyMakeBorder(
                        self.transformed_image, 0, ch - h, 0, 0,
                        cv2.BORDER_CONSTANT)
