...
class RetinaFace:
    ...
    def detect(self, im: np.ndarray, threshold: float = 0.6):
        ...
        t1 = time.time()
        # logger.debug(f"Retina inference took: {t1 - t0}")
    ...
    def postprocess(self, net_out, threshold):
        ...
        for _idx, s in enumerate(self._feat_stride_fpn):
            ...
            if self.masks:
                ...
                mask_scores = mask_scores.transpose((0, 2, 3, 1)).reshape(
                    (-1, 1))
        ...
        if self.masks:
            ...
            mask_scores = mask_scores[order]
            pre_det = np.hstack(
                (proposals[:, 0:4], scores, mask_scores)).astype(np.float32,
                                                                 copy=False)
        else:
        ...
        t1 = time.time()
        # logger.debug(f"Retina postprocess took: {t1 - t0}")
